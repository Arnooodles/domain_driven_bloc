...
class LoginBloc extends Cubit<LoginState> with BlocPresentationMixin<LoginState, LoginPresentationEvent> {
  LoginBloc(this._authRepository, this._localStorageRepository) : super(LoginState.initial()) {
    initialize();
  }
  ...
  Future<void> initialize() async {
    final String? username = await _localStorageRepository.getLastLoggedInUsername();
  ...
  Future<void> login(String username, String password) async {
    try {
      ...
      if (validUsername.isValid() && validPassword.isValid()) {
        final Either<Failure, Unit> possibleFailure = await _authRepository.login(validUsername, validPassword);
  ...
  void _emitFailure(Failure failure) {
    safeEmit(state.copyWith(isLoading: false));
    emitPresentation(LoginPresentationEvent.onFailure(failure));
  }
